...
def _wrap_by_canvas(
    ...
    for raw in raw_lines:
        ...
        if not words:
            lines_out.append("")
            continue
        budget_chars = max(1, int(content_width_px / max(1.0, approx_char_px)))
...
def generate_svg():
    ...
    approx_char_px = float(wrap_char_px) if wrap_char_px is not None else lh_px * float(wrap_ratio if wrap_ratio is not None else 0.18)
    ...
    lines = _wrap_by_canvas(norm_lines_in, content_width_px, max_chars_per_line=75, approx_char_px=approx_char_px)
...
def batch_generate():
    ...
    for idx, row in df.fillna("").iterrows():
        ...
        try:
            ...
            approx_char_px = float(wrap_char_px) if wrap_char_px not in (None, "") else lh_px * float(wrap_ratio if wrap_ratio not in (None, "") else 0.18)
            lines = _wrap_by_canvas(norm_lines_in, content_width_px, max_chars_per_line=75, approx_char_px=approx_char_px)
            if not any(line.strip() for line in lines):
...
def template_csv():
    header = (
        "filename,text,page_size,units,page_width,page_height,margins,line_height,align,background,global_scale,orientation,biases,styles,stroke_colors,stroke_widths\n"
    )
    ...
    def gen():
        ...
        for idx, row in df.fillna("").iterrows():
            ...
            try:
                ...
                generated_files.append(out_path)
                yield _sse({"type": "row", "status": "ok", "row": int(idx), "file": os.path.basename(out_path)})
            except Exception as e:
                errors.append((int(idx), str(e)))
                yield _sse({"type": "row", "status": "error", "row": int(idx), "error": str(e)})
